...
<div class="wrap logichop-insights">
	...
	<h1><?php _e('Logic Hop Insights', 'logichop'); ?></h1>
	
	<?php if (!$_SESSION['logichop']) : ?>
		<div class="notice notice-error">
			<p>
				Logic Hop Insights are an optional feature available with Logic Hop API access for data storage. <a href="https://logichop.com" target="_blank">Learn more</a>.
			</p>
		</div>
	<?php endif; ?>
	...
	<div class="row charts">
		...
		<div class="col-sm-4">
			<canvas class="chart" id="pages" width="400" height="400" data-title="<?php _e('Pages Tracked', 'logichop'); ?>"></canvas>
		</div>
		<div class="col-sm-2"></div>
  				...
  				<?php
  					if ($pages && $insights['Goals']) {
						...
						printf('<tr>
  								...
  								__('<strong>No Goals Triggered</strong>', 'logichop')
  							);
  					}		
  				?>
			</table>
		...
		<div class="col-xs-12">
			<table class="table table-striped table-bordered table-condensed">
  				<tr style="background: <?php printf('rgba(%s,%s,%s,1);', $page_color[0], $page_color[1], $page_color[2]); ?>;">
  					<th><?php _e('Pages Tracked', 'logichop'); ?></th>
  				...
  				<?php
  					if ($pages && $insights['Pages']) {
						...
						printf('<tr>
  								...
  								__('<strong>No Pages Tracked</strong><br>Select <em>"Enable Page/Post Tracking"</em> from the editor to track a specific page or post.', 'logichop')
  							);
  					}	
  				?>
			</table>
