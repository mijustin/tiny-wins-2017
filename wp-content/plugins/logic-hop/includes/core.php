...
class LogicHop_Core {
	 ...
	 */
	public function __construct ( $plugin_name, $version, $cookie_ttl, $path_max, $api_url, $debug = false ) {
	...
	public function initialize_core () {
		...
		do_action('logichop_integration_init');
	...
	public function validate_api () {
		...
		if ($this->get_option('api_key', false)) {
	...
	public function session_create () { 
		...
		$_SESSION['logichop-data']->IsBot	= $CrawlerDetect->isCrawler();
		
		$_SESSION['logichop-data']->IP 			= $this->get_client_IP();
		$_SESSION['logichop-data']->Location 	= null;
		$_SESSION['logichop-data']->LandingPage = '';
		$_SESSION['logichop-data']->LeadScore 	= 0;
		$_SESSION['logichop-data']->Source 		= '';
		$_SESSION['logichop-data']->UserAgent	= (isset($_SERVER['HTTP_USER_AGENT'])) ? $_SERVER['HTTP_USER_AGENT'] : '';;
	...
	public function cookie_create () {
		setcookie('logichop', $this->hash, $this->cookie_expires, $this->cookie_path, $this->domain);
	}
	...
	public function data_retrieve () {
		...
		if ($data) {
			...
			if (isset($data['LeadScore'])) 		$_SESSION['logichop-data']->LeadScore 		= (int) $data['LeadScore'];
			...
			if (isset($data['Location'])) {
				...
				$_SESSION['logichop-data']->Location = (object) $location;
			}
	...
	public function update_data ($pid = false, $referrer = false) {
		...
		if (isset($_SESSION['logichop-data']->Path)) {
		...
		if ($referrer !== false) {
			$_SESSION['logichop-data']->Referrer = $referrer;
		} else {
		...
		if (isset($_SESSION['logichop-data']->FirstVisit) && $_SESSION['logichop-data']->FirstVisit) {
			if ($_SESSION['logichop-data']->Source == '') {				
				...
				if ($this->js_tracking) {
					...
					$_SESSION['logichop-data']->LandingPage = (isset($landing_page)) ? $landing_page : '/';
				} else {
					$_SESSION['logichop-data']->LandingPage = strtok($_SERVER['REQUEST_URI'], '?'); // REMOVE QUERY STRING DATA
				}
				...
				$this->data_remote_put('user_agent', $_SESSION['logichop-data']->UserAgent); // STORE HTTP USER AGENT
				$this->data_remote_put('lead_score', $_SESSION['logichop-data']->LeadScore); // STORE LEAD SCORE
			}				
		}
		...
		if ($post_type == 'page' || $post_type == 'post') {
			$this->data_remote_put('page', $post_id);	// TRACK PAGE --> STORE PAGE VIEW FOR post OR page POST TYPES
		}
	 ...
	 *
	 * Used to lookup session variable for sending from Goal
	...
	public function update_goal ($goal_id = null) {
		if (is_int($goal_id)) {
			...
			if (isset($goal) && $goal->post_type == 'logichop-goals') {
				...
				$values	= get_post_custom($goal_id);
				...
				if (isset($values['logichop_goal_lead_score_adjust'][0]) && isset($values['logichop_goal_lead_freq'][0])) {
					...
					if ($lead_freq == 'every') {
						...
						if (isset($_SESSION['logichop-data']->Goals[$goal_id])) {
							if ($_SESSION['logichop-data']->Goals[$goal_id] == 1) {
								$this->update_lead_score($lead_adjust);
							}
						}
					} else if ($lead_freq == 'session') {
						if (isset($_SESSION['logichop-data']->GoalsSession[$goal_id])) {
							if ($_SESSION['logichop-data']->GoalsSession[$goal_id] == 1) {
								$this->update_lead_score($lead_adjust);
							}
						}
					}
				...
				$this->check_track_event($goal_id, $values);
	...
	public function validate_lead_score ($post_id, $lead_adjust, $lead_freq) {
		if ($lead_freq == 'every') {
			...
			if (isset($_SESSION['logichop-data']->Pages[$post_id])) {
				if ($_SESSION['logichop-data']->Pages[$post_id] == 1) {
					return $this->update_lead_score($lead_adjust);
				}
			}
		} else if ($lead_freq == 'session') {
			if (isset($_SESSION['logichop-data']->PagesSession[$post_id])) {
				if ($_SESSION['logichop-data']->PagesSession[$post_id] == 1) {
					return $this->update_lead_score($lead_adjust);
				}
			}
		}
	...
	public function update_lead_score ($lead_adjust) {
		if ($lead_adjust != 0 && is_numeric($lead_adjust)) {
			$lead_score = (isset($_SESSION['logichop-data']->LeadScore)) ? (int) $_SESSION['logichop-data']->LeadScore : 0;
			$new_lead_score = $lead_adjust + $lead_score;
			$_SESSION['logichop-data']->LeadScore = $new_lead_score;
			return $this->data_remote_put('lead_score', $new_lead_score, 'event-update');
		}
		return false;
	}
	 ...
	 */
	public function data_remote_put ($type, $value, $endpoint = 'event') {
		$data = array (
					'uid'		=> $_SESSION['logichop-data']->UID,
					'type'		=> $type,
					'value'		=> $value,
		...
		return $this->api_post($endpoint, $data);
	}
	...
	public function api_post ($endpoint, $data = array(), $key = false) {
		...
		if (in_array($endpoint, array('event'))) { 
			if ($_SESSION['logichop-data']->IsBot) {
