(function( $ ) {
  ...
  $(function() {
    ...
    completable_list = {};
    if ( localStorage && localStorage.getItem('wpcomplete.completable-list') && ( localStorage.getItem('wpcomplete.completable-list') != '' ) ) {
      completable_list = JSON.parse(localStorage.getItem('wpcomplete.completable-list'));
    }
    // but make sure we don't have an old and out of date cache... (no more than 24 hours)
    if ( completable_list && completable_list['timestamp'] && ( completable_list['timestamp'] > ((new Date).getTime()/1000 - (60*60*24)) ) ) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if ( localStorage && response['user'] && ( response['user'] > 0 ) ) { localStorage.setItem("wpcomplete.completable-list", JSON.stringify(response)); }
